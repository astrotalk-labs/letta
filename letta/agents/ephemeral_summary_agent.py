...
class EphemeralSummaryAgent(BaseAgent):
    ...
    async def step(self, input_messages: list[MessageCreate], max_steps: int = DEFAULT_MAX_STEPS) -> list[Message]:
        ...
        if use_azure:
            ...
            logger.warning(f"[SUMMARIZER] Using Anthropic for summarization | at_user_id={self.at_user_id} | agent_id={self.agent_id}")
        ...
        logger.warning(
            f"[SUMMARIZER] Summarization completed | at_user_id={self.at_user_id} | agent_id={self.agent_id} | provider={'azure' if use_azure else 'anthropic'} | summary_length={len(summary)}"
        )
