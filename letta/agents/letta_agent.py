...
class LettaAgent(BaseAgent):
    ...
    async def _step(
        ...
        for i in range(max_steps):
            ...
            # Snapshot the pre-call message state so we can re-run on the primary model if the
            # quality gate below rejects Haiku's choice of tool.
            ...
            # Quality gate: if the cascade attempted Haiku and Haiku picked anything other than
            # a memory tool (most commonly send_message, i.e. the user-facing reply), discard
            # the Haiku response and re-run the step on the primary model. This guarantees
            # send_message and any non-memory tool calls are always generated by the full model.
            if _step_used_haiku:
                _haiku_picked_tool = (
                ...
                if _haiku_picked_tool not in _MEMORY_TOOL_NAMES:
                    logger.warning(
                        f"[HAIKU_CASCADE] task_id={task_id or 'N/A'} step={i} RETRY_ON_PRIMARY "
                        f"haiku_picked={_haiku_picked_tool or 'text_response'} "
                        f"reason=preserve_quality_for_user_facing_response "
                    ...
                    # Re-run on the primary model. The model is already restored above, so this
                    # call naturally uses the primary. Reuse the pre-call message state to avoid
