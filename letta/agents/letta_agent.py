...
# Tools that must always be generated by the primary (Sonnet) model — never Haiku.
# `send_message` is the user-facing reply tool; its text quality directly impacts the
# user experience, so we always re-run on the primary model if Haiku picks it.
_SONNET_RESERVED_TOOLS: frozenset = frozenset({
    "send_message",
})
...
class LettaAgent(BaseAgent):
    ...
    async def _step(
        ...
        if latency_optimisation_flow:
            ...
            if _haiku_model:
                logger.warning(
                    ...
                    f"rule=tool_calls_on_haiku_send_message_on_primary "
                    f"sonnet_reserved_tools={sorted(_SONNET_RESERVED_TOOLS)}"
                )
            else:
                logger.warning(
        ...
        for i in range(max_steps):
            ...
            # other tool in _SONNET_RESERVED_TOOLS), guaranteeing that the user-facing reply
            ...
            # Quality gate: if the cascade attempted Haiku and Haiku produced the user-facing
            # reply (picked send_message, or returned text that would be wrapped as a synthetic
            # send_message), discard the Haiku response and re-run the step on the primary
            # model. Tool calls other than send_message are kept on Haiku.
            if _step_used_haiku:
                _haiku_picked_tool = (
                ...
                # Retry when Haiku picked a primary-reserved tool (send_message), or returned
                # no tool at all (text response → would be wrapped as synthetic send_message).
                _needs_primary_retry = (
                    _haiku_picked_tool in _SONNET_RESERVED_TOOLS
                    or _haiku_picked_tool is None
                )
                if _needs_primary_retry:
                    logger.warning(
                        ...
                        f"reason=user_facing_reply_must_use_primary_model "
