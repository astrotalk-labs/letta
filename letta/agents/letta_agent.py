...
# Ramp 50 -> 100 as production metrics confirm latency improvement; set to 0 for an
# instant full rollback. The always-on test user is honored regardless of the percentage.
...
class LettaAgent(BaseAgent):
    ...
    async def _step(
        ...
        for i in range(max_steps):
            ...
            _step_llm_ms: Optional[int] = None  # captured for USAGE log (Haiku vs primary comparison)
            ...
            try:
                ...
                if result is not None:
                    ...
                    if _llm_start:
                        _step_llm_ms = ns_to_ms(get_utc_timestamp_ns() - _llm_start)
                        if task_id:
                            logger.warning(f"[TASK_LATENCY] task_id={task_id} step={i} phase=llm_call duration_ms={_step_llm_ms}")
                    in_context_messages = current_in_context_messages + new_in_context_messages
            ...
            if _haiku_timed_out:
                ...
                if _llm_timeout_retry_start:
                    _step_llm_ms = ns_to_ms(get_utc_timestamp_ns() - _llm_timeout_retry_start)
                    if task_id:
                        logger.warning(
                            f"[TASK_LATENCY] task_id={task_id} step={i} phase=llm_call_timeout_fallback "
                            f"duration_ms={_step_llm_ms}"
                        )
                in_context_messages = current_in_context_messages + new_in_context_messages
                response = llm_client.convert_response_to_chat_completion(response_data, in_context_messages, agent_state.llm_config)
