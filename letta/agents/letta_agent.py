...
class LettaAgent(BaseAgent):
    ...
    async def _step(
        ...
        for i in range(max_steps):
            ...
            # Quality gate (post tool-restriction): Haiku's tools never included send_message,
            # so it can only have produced either (a) some other tool call — which we keep, or
            # (b) a text response, which means Haiku had nothing it could legitimately call and
            # is signalling "ready to answer". In case (b) we discard and retry on primary so
            # the actual send_message is generated by the primary model.
            if _step_used_haiku:
                _haiku_has_tool_call = bool(response.choices[0].message.tool_calls)
                if not _haiku_has_tool_call:
                    logger.warning(
                        f"[HAIKU_CASCADE] task_id={task_id or 'N/A'} step={i} RETRY_ON_PRIMARY "
                        f"haiku_picked=text_response "
                        f"reason=haiku_exhausted_tool_options_hand_off_for_send_message "
